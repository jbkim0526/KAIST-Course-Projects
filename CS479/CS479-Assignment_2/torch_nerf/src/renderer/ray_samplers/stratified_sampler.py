...
class StratifiedSampler(RaySamplerBase):
    ...
    def sample_along_rays_uniform(
        ...
        near, far = ray_bundle.nears[0].item(), ray_bundle.fars[0].item()
        num_ray, device = ray_bundle.nears.shape[0], ray_bundle.nears.device
        t_bins = self.create_t_bins(num_sample+1,device)[:-1].expand(num_ray,num_sample)
        t_rand = torch.rand_like(t_bins) * (1/num_sample)
        t = t_bins + t_rand
        t_samples = self.map_t_to_euclidean(t,near,far)
        return t_samples
