...
class RaySamples:
    ...
    def compute_sample_coordinates(self) -> Float[torch.Tensor, "num_ray num_sample 3"]:
        ...
        origins = self.ray_bundle.origins.unsqueeze(1).expand(num_ray,num_sample,3)
        directions = self.ray_bundle.directions.unsqueeze(1).expand(num_ray,num_sample,3)
        t_samples = self.t_samples.unsqueeze(-1).expand(num_ray,num_sample,3)
        sample_coords = origins + directions * t_samples
