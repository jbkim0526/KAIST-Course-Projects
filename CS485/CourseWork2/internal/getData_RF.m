function [ data_train, data_query, test_img_idx, numBins, numT,depth,split] = getData_RF( MODE )
...
switch MODE
    case 'Caltech' % Caltech dataset
        ...
        [desc_sel,y] = vl_colsubset(cat(2,desc_tr{:}), 10e4); % Randomly select 100k SIFT descriptors for clustering
...
switch MODE
    case 'Caltech'
        ...
        % ...
        numBins = 2^(param.depth-1);
        histogram_te = zeros(length(classList) * imgSel(2), numBins * param.num);
        cnt = 1;
        for c = 1:length(classList)
            for i = 1:imgSel(2)
                %[~, ~, quantized_descriptors] = predict(tree, single(desc_te{c,i}'));
                desc_te_size = size(desc_te{c,i});
                z = cnt* ones(1, desc_te_size(2));
                test_data = cat(1,desc_te{c,i},z);
                leaves = reshape(testTrees_fast(test_data',trees), [], 1);
                histogram_te(cnt, :) = histcounts(leaves, 1: numBins*param.num + 1);
                cnt = cnt + 1;
            end
        end
        % ...
